...
const GetTemperatureIntentHandler = buildIntentHandler("GetTemperatureIntent", (response, device_num, locale) => {
    ...
    speechText = stringReplace(strings[locale].temperatureIntent.speechText, [{"needle": "{{value}}", "replacement": temperature}]);
...
const GetAirQualityIntentHandler = buildIntentHandler("GetAirQualityIntent", (response, device_num, locale) => {
    ...
    let time_diff = Math.floor(Date.now() / 1000) - time_utc;
    speechText = stringReplace(strings[locale].airQualityIntent.speechText, [{"needle": "{{value}}", "replacement": co2}]);
...
const GetNoiseIntentHandler = buildIntentHandler("GetNoiseIntent", (response, device_num, locale) => {
    ...
    let time_diff = Math.floor(Date.now() / 1000) - time_utc;
    speechText = stringReplace(strings[locale].noiseLevelIntent.speechText, [{"needle": "{{value}}", "replacement": noise}]);
    card.message = stringReplace(strings[locale].humidityIntent.card.message,
        [{"needle": "{{speechText}}", "replacement": speechText}, {"needle": "{{timeLeft}}", "replacement": timeLeft(time_diff, locale)}]);
    return {"speechText": speechText, "card": card};
});
...
const GetPressureIntentHandler = buildIntentHandler("GetPressureIntent", (response, device_num, locale) => {
    ...
    let time_diff = Math.floor(Date.now() / 1000) - time_utc;
    speechText = stringReplace(strings[locale].airPressureIntent.speechText, [{"needle": "{{value}}", "replacement": pressure}]);
    card.message = 
        stringReplace(strings[locale].airPressureIntent.card.message, [{"needle": "{{air_pressure}}", "replacement": pressure}, {"needle": "{{abs_air_pressure}}", "replacement": absPressure}]);
    return {"speechText": speechText, "card": card};
});
...
const GetLastUpdateIntentHandler = buildIntentHandler("GetLastUpdateIntent", (response, device_num, locale) => {
    ...
    const time_utc = response.body.devices[device_num].dashboard_data.time_utc;
    let time_diff = Math.floor(Date.now() / 1000) - time_utc;
    speechText = stringReplace(strings[locale].timeIntent.speechText, [{"needle": "{{value}}", "replacement": timeLeft(time_diff, locale)}]);
...
const GetDeviceListIntentHandler = buildIntentHandler("GetDeviceListIntent", (response, device_num, locale) => {
    ...
    devices.forEach((it) => {
        speechText += stringReplace(strings[locale].deviceListIntent.stationName, [{"needle": "{{station_name}}", "replacement": count}]);
